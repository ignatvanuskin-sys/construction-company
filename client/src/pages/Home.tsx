...
<section id="projects" className="projects"><SectionLabel no="03" children="Избранные проекты"/><div className="section-heading"><h2>Пространства<br/><em>с характером</em></h2><a className="text-link" href="#contact">Запросить портфолио <ArrowUpRight size={16}/></a></div><div className="project-grid">{projects.map((p,i)=><Link href={`/projects/${p.slug}`} onMouseEnter={() => trackEvent("project_card_hover", { project: p.slug })} className={`project-card card-${i+1}`} key={p.slug}><div className="project-image"><img src={p.image} alt={p.name}/><span className="project-tag">{p.tag}</span><span className="project-arrow"><ArrowUpRight size={20}/></span></div><div className="project-meta"><span>{p.no}</span><div><h3>{p.name}</h3><p>{p.type} · {p.meta}</p></div></div></Link>)}</div></section>
